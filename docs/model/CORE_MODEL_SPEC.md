...
**What's settled vs. open.** Settled: the structured-reference grammar (§3.3, adopted from Excel verbatim), that Tables are not an OxFunc value, the first persistent table-node fixture shape in `docs/test-corpus/workspaces/tables.json`: stable table id, identity policy for rename/move/delete, row ids/order, column ids/names/ordinals, header/totals presence, body constants or column formulas, totals formulas, and explicit version fields for namespace, row membership, row order, and column identity, and the first LiveOxCalc bridge projection of that fixture shape into OxCalc's W056 table-node/table-catalog surface. The bridge now returns table-context identities with engine results so the host can correlate a table node to OxCalc's generic table packet without reconstructing structured-reference semantics. Open and owned cross-repo (engine prerequisite §6 item 14, [`../handovers/HANDOVER_OXCALC_table_node_model.md`](../handovers/HANDOVER_OXCALC_table_node_model.md)): table reference readers, per-row column-formula runtime, how `[#Headers]` / `[#Data]` / `[#Totals]` select slices in retained product evidence, the dependency edges a table reference creates, and the per-cell value surface returned to the host. The table editor and Excel export rest on that engine behavior, not on host reconstruction.

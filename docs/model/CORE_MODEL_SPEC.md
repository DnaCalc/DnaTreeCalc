...
**Per-node observable value is either `Empty` or an `EvalValue`.** `Empty` is the value of a node whose formula text is the empty string. It is not a formula-evaluation result: no `=...` formula, array formula, or non-empty literal entry can evaluate a node to top-level `Empty`. Formula output may be an empty string (`""`), which is a text value and is distinct from `Empty`. Dereferencing a reference to an empty node can return the `Empty` value to a caller, matching Excel's blank-cell distinction. This maps onto OxCalc's reference-reader model (W051): a node's `Empty` is the reader's `Blank`, while a node whose formula outputs `""` is `Defined(Text(""))` — so `SUM` / `COUNT` / `COUNTA` / `COUNTBLANK` over `@CHILDREN` behave exactly as over a worksheet range (Excel-anchored in OxFunc).
...
3. **Set-membership dependency edge type** in OxCalc's `DependencyGraph`: a formula depending on a node's child collection must invalidate on the dependency triple — **membership** (add / remove / rename of a matching child), **sibling-order change**, and **current member-value reads**; meta-effective children are excluded. OxCalc **W051** delivers this first for `@CHILDREN` / `.*`; the same machinery extends later to `@ANCESTORS`, `@PRECEDING`/`@FOLLOWING`, `**`, and explicit `{…}` literals (not in W051's first scope).
...
12. **Circular-reference cycle profiles** — the host selects a cycle profile and supplies iterative bounds (Maximum Iterations, Maximum Change) via the typed `cycle_config` field on `OxCalcTreeContext edit/recalc call` (OxCalc consumer contract §6.3, W055 — **not** the compatibility basis); OxCalc owns the profiles and the iteration (`docs/spec/core-engine/w048-cycles/`): `cycle.non_iterative_stage1` (default — reject / `cycle_blocked`), `cycle.excel_match_iterative` (Excel-faithful for the current single-host-scoped covered surface), `cycle.iterative_deterministic_v0` (deterministic Jacobi). Cycle outcomes return in the typed `cycle_diagnostics` result field (§6.4). See §7a and `docs/handovers/HANDOVER_OXCALC_iterative_cycle_config.md`.
...
A node awaiting a **host-driven completion** — an async function, an RTD value, or another external update not yet processed — displays an **async-pending** state (the `#GETTING_DATA` analog), distinct from `evaluating` (actively computing): it is suspended, awaiting the host to feed the completion. The engine surfaces this as return-value data (a pending run-state + completion tokens), never a callback; the host resumes by calling in, exactly like F9 (see [`../handovers/HANDOVER_OXCALC_engine_handle_and_incremental_edit.md`](../handovers/HANDOVER_OXCALC_engine_handle_and_incremental_edit.md) §3). This is anticipated contract shape — async functions are not on the v1 path.
...
**Cycle profiles** (OxCalc W048; selected via the typed `cycle_config` field on the recalc request — OxCalc consumer contract §6.3, W055):
...
These mirror Excel's *File ▸ Options ▸ Formulas ▸ Enable iterative calculation / Maximum Iterations / Maximum Change*. The default is non-iterative (`cycle.non_iterative_stage1`), matching Excel out of the box. The selected profile and bounds are conveyed to OxCalc through the typed `cycle_config` field on `OxCalcTreeContext edit/recalc call` (OxCalc consumer contract §6.3, W055); the **compatibility basis is explicitly not the cycle channel**. Cycle outcomes return in the typed `cycle_diagnostics` result field (§6.4), with a typed `Worksheet.CircularReference` equivalent for the non-iterative case — hosts read these typed facts rather than parsing string diagnostics. **TreeCalc does not implement iteration itself**.

**Calc-state and UX.** Under the non-iterative default, cycle members display `cycle_blocked` (§7) and surface in the workspace error summary and the "show cycle" affordance (`ux/REQUIREMENTS.md`). Under an iterative profile, members display their iterated values, and the engine's iteration trace (within `cycle_diagnostics`, §6.4) is available for diagnostics.
...
**Template storage and bookkeeping.** A template lives as a meta-flagged subtree in the workspace tree — a node with `is_meta = true` and the descendants that form the template's structural pattern. Templates can be placed at any tree position (workspace root, sheet level, deeper) — host convention typically groups them under a `Templates` container at some level. The template's nodes and their descendants are invisible to formulas; their formulas are stored as text but not bound or evaluated. See [`META_NODES.md`](META_NODES.md) for the meta-node model.

The host also keeps a derived template index: template id, template root node id, version, child/source-node ids, and instance counts/links. This index is not a second copy of the template body; it is the host's bookkeeping over the meta-subtree, its children, and rollout tags so the UI can list templates, find rollouts, and validate mappings cheaply.
...
**What's settled vs. open.** Settled: the structured-reference grammar (§3.3, adopted from Excel verbatim), that Tables are not an OxFunc value, the persistent table-node fixture shape in `docs/test-corpus/workspaces/tables.json`: stable table id, identity policy for rename/move/delete, row ids/order, column ids/names/ordinals, header/totals presence, body constants or column formulas, totals formulas, and explicit version fields for namespace, row membership, row order, and column identity, and the direct OxCalc context projection of that fixture shape into OxCalc's W056 table-node/table-catalog surface. `OxCalcTreeContext` returns table-context identities with engine results so the host can correlate a table node to OxCalc's generic table packet without reconstructing structured-reference semantics. The active table corpus runner (`src/dnatreecalc-host/tests/active_table_corpus.rs`) now validates `Table[Col]`, `Table[@Col]`, omitted `[@Col]`, `[#Headers]`, `[#Data]`, `[#Totals]`, `[#All]`, composite refs, table column formulas, totals formulas, table update impact, zero-row headers-only and headers+totals bodies, first-row insert and last-row delete endpoints, current-row diagnostics in an empty data body, and the full table lifecycle callback event set through DnaTreeCalc's table fixture projection, OxFml structured-reference bind records, and OxCalc's public W056 table reader/runtime/update/lifecycle APIs. Bracket-escaped TreeCalc table-path forms remain an OxFml packet blocker and are recorded as typed pending evidence rather than DnaTreeCalc parsing. The lifecycle path covers create/delete, rename/move, row/column/header/totals/body/formula events, save/reopen JSON round-trip of stable table/node/row/column handles, and stale table-identity rejection. The retained replay producer artifact in `docs/test-runs/w056-table-structured-references-001/` exposes the primary `SalesTable` update/evidence slice as OxReplay-facing `table_slice`, `per_node_value`, `effective_display_text`, `execution_outcome`, `dependency_evidence`, `invalidation_evidence`, and `retained_artifact_ref` views generated from the same direct table projection; the artifact source metadata explicitly keeps bracket-escaped table retained-slice capture out of this first retained run while the active corpus records those cases as OxFml packet blockers. Still open for later product closure: wiring these retained table values into the ordinary user-facing tree shell/editor and the W009 Excel export/replay path, plus retaining the new zero-row table evidence through OxXlPlay/OxReplay. The table editor and Excel export rest on engine behavior, not on host reconstruction.

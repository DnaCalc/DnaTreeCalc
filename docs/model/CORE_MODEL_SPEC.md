...
12. **Circular-reference cycle profiles** — the host selects a cycle profile and supplies iterative bounds (Maximum Iterations, Maximum Change) via the typed `cycle_config` field on `OxCalcTreeRecalcRequest` (OxCalc consumer contract §6.3, W055 — **not** the compatibility basis); OxCalc owns the profiles and the iteration (`docs/spec/core-engine/w048-cycles/`): `cycle.non_iterative_stage1` (default — reject / `cycle_blocked`), `cycle.excel_match_iterative` (Excel-faithful for the current single-host-scoped covered surface), `cycle.iterative_deterministic_v0` (deterministic Jacobi). Cycle outcomes return in the typed `cycle_diagnostics` result field (§6.4). See §7a and `docs/handovers/HANDOVER_OXCALC_iterative_cycle_config.md`.
...
These mirror Excel's *File ▸ Options ▸ Formulas ▸ Enable iterative calculation / Maximum Iterations / Maximum Change*. The default is non-iterative (`cycle.non_iterative_stage1`), matching Excel out of the box. The selected profile and bounds are conveyed to OxCalc through the typed `cycle_config` field on `OxCalcTreeRecalcRequest` (OxCalc consumer contract §6.3, W055); the **compatibility basis is explicitly not the cycle channel**. Cycle outcomes return in the typed `cycle_diagnostics` result field (§6.4), with a typed `Worksheet.CircularReference` equivalent for the non-iterative case — hosts read these typed facts rather than parsing string diagnostics. **TreeCalc does not implement iteration itself**.
...
**What's settled vs. open.** Settled: the structured-reference grammar (§3.3, adopted from Excel verbatim), that Tables are not an OxFunc value, the persistent table-node fixture shape in `docs/test-corpus/workspaces/tables.json`: stable table id, identity policy for rename/move/delete, row ids/order, column ids/names/ordinals, header/totals presence, body constants or column formulas, totals formulas, and explicit version fields for namespace, row membership, row order, and column identity, and the LiveOxCalc bridge projection of that fixture shape into OxCalc's W056 table-node/table-catalog surface. The bridge returns table-context identities with engine results so the host can correlate a table node to OxCalc's generic table packet without reconstructing structured-reference semantics. The active table corpus runner (`src/dnatreecalc-host/tests/active_table_corpus.rs`) now validates `Table[Col]`, `Table[@Col]`, omitted `[@Col]`, `[#Headers]`, `[#Data]`, `[#Totals]`, `[#All]`, composite refs, bracket-escaped table and column names, table column formulas, totals formulas, table update impact, and the full table lifecycle callback event set through DnaTreeCalc's table fixture projection plus OxCalc's public W056 table prebind/reader/runtime/update/lifecycle APIs. The lifecycle path covers create/delete, rename/move, row/column/header/totals/body/formula events, save/reopen JSON round-trip of stable table/node/row/column handles, and stale table-identity rejection. The retained replay producer artifact in `docs/test-runs/w056-table-structured-references-001/` exposes the primary `SalesTable` update/evidence slice as OxReplay-facing `table_slice`, `per_node_value`, `effective_display_text`, `execution_outcome`, `dependency_evidence`, `invalidation_evidence`, and `retained_artifact_ref` views generated from the same live table projection; the artifact source metadata explicitly keeps bracket-escaped table retained-slice capture out of this first retained run while the active corpus still exercises those cases. Still open for later product closure: wiring these retained table values into the ordinary user-facing tree shell/editor and the W009 Excel export/replay path; activating zero-row table bodies through the real bridge is also blocked until OxCalc can project empty data bodies instead of returning `TreeCalcTableProjectionError::EmptyDataBodyNotRepresentableByCurrentTableDescriptor`. The pending `tables/empty-body` corpus records headers-only, headers+totals, first-row insert, last-row delete, and `#Data` / `[Col]` / `#All` / `#Headers` / `#Totals` / `[@Col]` diagnostic cases without reimplementing formula or dependency classification host-side. The table editor and Excel export rest on engine behavior, not on host reconstruction.

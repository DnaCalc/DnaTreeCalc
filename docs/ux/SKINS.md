...
- `ValueBoard` consumes that projection directly for table highlighting, keyboard table-cell navigation, selected-cell summaries, a compact active-selection strip, and a structured active-detail panel with stable node keys, node content/calc-state rows, selected table-cell formula text when present, dependency counts, bounded dependency-handle summaries, and published reference-target summaries. It does not derive table semantics from formula text or re-resolve structured references.

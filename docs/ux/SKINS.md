...
- `WorkspaceState` carries the skin-facing projection for tree nodes, values, calc state, last run status, diagnostics, dependency graph summaries, invalidation summaries, table identity/lifecycle summaries, and active table-cell detail readback for body/totals cells, including formula metadata when the active cell is formula-backed.

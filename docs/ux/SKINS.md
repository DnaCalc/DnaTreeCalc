...
- `WorkspaceState` carries the skin-facing projection for tree nodes, values, content kind, calc state, last run status, diagnostics, dependency graph summaries, invalidation summaries, runtime overlay summaries, table identity/lifecycle summaries, active table-cell detail readback for body/totals cells including formula/dependency/editability metadata, and unified active-selection detail so skins can inspect the focused node or table cell through one typed read path.

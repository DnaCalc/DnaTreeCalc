...
- `WorkspaceState` carries the skin-facing projection for tree nodes, values, content kind, calc state, last run status, diagnostics, dependency graph summaries, invalidation summaries, runtime effect/overlay summaries, table identity/lifecycle summaries, active table-cell detail readback for body/totals cells including formula/dependency/editability metadata, and unified active-selection detail so skins can inspect the focused node or table cell through one typed read path.
...
- `ValueBoard` consumes that projection directly for table highlighting, keyboard table-cell navigation, selected-cell summaries, a compact active-selection strip, and a structured active-detail panel backed by typed skin-local rows with stable labels and row ids. The panel includes stable node keys, node content/calc-state rows, selected table-cell formula text when present, dependency counts, bounded dependency-handle summaries, published reference-target summaries, published source spans, run-level effect/diagnostic/timing summaries, and selected-node last-run invalidation/derivation-trace/overlay context. It does not derive table semantics from formula text or re-resolve structured references.
- The shell and lens stylesheets (`dnatreecalc-shell`'s `SHELL_CSS`/`COCKPIT_CSS`, plus the per-lens CSS constants) carry the responsive + touch contract (bead dtc-ajl.32): the shell grid pins `grid-template-columns: minmax(0, 1fr)` so a lens with huge intrinsic width (the demo grid's full-bounds canvas) can never drag the page horizontally; the header, skin switcher, and status foot wrap instead of overflowing; companion slots stack under the main lens and the multi-column skins (`triple-editor`, `formula-tree`, `dependency-inspector`, notebook rail) collapse to one column at `≤900px`; the shortcut bar stands down at `≤700px` and under `pointer: coarse`, where chrome controls grow to ≥2.25rem targets and form text to 16px to prevent mobile focus-zoom; formula/content editors set `autocapitalize="off"`/`spellcheck="false"` so soft keyboards do not mangle identifiers.

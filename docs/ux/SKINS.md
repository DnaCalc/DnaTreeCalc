...
- `WorkspaceState` carries the skin-facing projection for tree nodes, values, calc state, last run status, diagnostics, dependency graph summaries, invalidation summaries, table identity/lifecycle summaries, active table-cell detail readback for body/totals cells including formula/dependency/editability metadata, and unified active-selection detail so skins can inspect the focused node or table cell through one typed read path.
...
- `ValueBoard` consumes that projection directly for table highlighting, keyboard table-cell navigation, selected-cell summaries, and a unified active-selection strip with dependency counts. It does not derive table semantics from formula text or re-resolve structured references.

...
| G11 | **Reference resolution-step projection** — a reference's engine-true resolution as ordered discrete steps (walk-up hops, anchor hop, selector application, collection fan-out), beyond today's span+target, so the Model Reference X-Ray replays the TRUE intermediate hops rather than interpolating them | `ReferenceResolutionProjection` carries only `token_span`, `target` (Node/Collection/External/Unresolved), `descriptor_ids`, kinds — no step list (`skin-ir/src/workspace.rs:2210-2241`) | Model Reference X-Ray engine-true replay (S4 F6/M8) · mech 07 · S4 | OxFml/OxCalc resolver emits steps + IR + host-core |

> **G11 filed 2026-07-23 (owner decision).** For the Reference X-Ray, the owner chose engine-true resolution steps over the presentational interpolation the plan originally proposed, and to file the ask NOW (not at V2) and build M8 against it. G11 therefore gates S4 **F6** (`xray.rs` hop derivation) and **M8** (X-Ray replay). Per the no-shims discipline, the honest pre-G11 degrade — if M8 must land before G11 arrives — is presentational hops (owner-key→resolved-target-key/member-keys interpolation) with a visible "not engine-true steps yet (G11)" note; the ambition and the ask, however, are engine-true. Still needs filing into the official ledger `docs/ux/stack-requirements/HOST_AND_SKIN_IR_REQUIREMENTS.md` + the upstream OxFml/OxCalc resolver lane (S4-kickoff action).

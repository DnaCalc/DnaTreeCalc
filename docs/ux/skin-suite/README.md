...
| **B.2 — time-to-result + flow control** | **B.2.0** engine perf rounds 1+2 (n≤1000 interactive); **B.2.1** serializable IR seam; **B.2.2–B.2.4** the worker boundary — wire protocol, `apply_delta`/resync, coalescing/telemetry, and the `WorkerProxyCore` state machine — **built + unit-tested in-process**. *Detailed plan: [`PHASE_B.md`](PHASE_B.md)* | **Built (boundary); live `web_sys::Worker` wiring tracked** — needs the executor decoupled from Leptos; non-urgent while n≤1000 is interactive |
| **B.3 — governance + reach** | Preview seam + **impact previews**, reviewer persona + **governed catalog**, intent-log replay, multi-select promotion, **keybinding v2 override layer**, **cockpit split panes + user presets**. *Detailed plan: [`PHASE_B.md`](PHASE_B.md)* | **Built** (reach tier); B.3.7 hardening/polish deferred |

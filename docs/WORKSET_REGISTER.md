...
- **Depends on:** W005 (the skeleton's real bridge + walk-up base). Engine prereqs (SelfNode, set-membership deps, profile selectors, dynamic refs, reference-array carriers, node-as-function binding, table structured-ref lowering) via handover.
...
- **Current bridge note:** the live bridge now consumes OxCalc's public raw TreeCalc prebind for original `=SUM(@CHILDREN)` and `=SUM(.*)` formula text, producing the `ChildrenV1` carrier through the real OxCalc/OxFml/OxFunc path. The first narrow JSON-backed active slice is `references/children-raw-active`: its Rust runner loads the active corpus theme, executes the fixture through `LiveOxCalcTreeBridge`, and asserts published values plus dependency membership for only those two free-standing raw children formulas. The broad `references/set-membership` family remains `pending`; the public prebind surface does not yet cover broader raw formula families (`base.@CHILDREN`, walk-up names such as `=A+3`, tables, node-as-function calls, recursive/walk-up collections, etc.; tracked by `dtc-osq.2`).
- **Status:** OPEN

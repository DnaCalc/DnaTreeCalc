...
- **Current bridge note:** a narrow carrier-backed OxCalc/OxFml slice now runs opaque formula source with a typed `ChildrenV1` reference carrier, but original TreeCalc user formula text still cannot be activated without the upstream host-reference parse/bind interface (`dtc-osq.2`). Corpus themes remain `pending` until the runner exercises original source text through that interface.
- **Status:** OPEN

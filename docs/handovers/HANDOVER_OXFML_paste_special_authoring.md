...
|---|---|---|
| Literalize computed value for paste | typed `CalcValue`/array value, target capability/profile context, locale/format policy where relevant | authored input text or explicit unsupported verdict, diagnostics/profile violations, provenance that this is value literalization rather than display text |
...
Until these APIs exist, DnaTreeCalc should keep full computed-value paste, array paste, formula paste, formula-and-format paste, duplicate-subtree formula rebind, and cut source deletion that depends on a successful rebind out of the supported Skin IR surface. Constant-source value paste remains supported because it uses authored source input text, not rendered computed value text.

...
| `tables/empty-body` | W004 pending empty-body table runner | DnaTreeCalc owns headers-only and headers+totals product table fixtures, plus first-row insert and last-row delete transition endpoints. The runner records the exact current upstream blocker for zero-row bodies (`TreeCalcTableProjectionError::EmptyDataBodyNotRepresentableByCurrentTableDescriptor`) and exercises the one-row transition endpoints through `LiveOxCalcTreeBridge`; the theme stays `pending` until OxCalc/OxFml can project zero-row tables and execute `#Data`, `[Col]`, `#All`, `#Headers`, `#Totals`, and `[@Col]` diagnostics through the real bridge. |

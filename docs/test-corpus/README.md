...
| `tables/structured-references` | W004 active table runner | Structured refs lower through DnaTreeCalc's table fixture projection and OxCalc's public W056 table runtime path; TreeCalc asserts target kind, selected columns, row-context values, formula/totals results, dependency lowering, update impact, and bridge-retained table context identity. |

...
- **Not a grid.** No coordinates; no value spilling between nodes. The grid arrives with PreCalc and beyond. This clause describes the TreeCalc **model**, not the repo's hosting scope. Under the owner-approved dual-profile decision (`treecalc-v1` + `strict-excel-grid`) and the W011 DnaCalc-host pivot, this repo hosts **both** tree workspaces and `.xlsx` grid workbooks through `dnacalc-host-core`. The grid profile is real, not a rewrite of the tree thesis: the W011 fixture `fixtures/w011/a1_times_three` (`A1 = 7`, `B1 = =A1*3` with stale cached `21`) is opened through OxDoc, edited (`A1` → `10`), recalculated by OxCalc (`B1` → `30`), saved through OxDoc, and reopened with `B1`'s formula text preserved **and** its cached value refreshed to `30` — pinned by `workbook::tests::save_after_edit_reopens_with_cached_30` and `tests::execute_save_active_xlsx_after_edit_returns_bytes_with_cached_30` (`cargo test -p dnacalc-host-core --offline`, 74 passed at commit `0e7d60a`, bead `dtc-j7n8.7`). Details: `docs/WORKSET_REGISTER.md` "W011 pivot" and `docs/ux/DNACALC_HOST_CORE_XLSX_NOTEBOOK_PROOF.md`. *(Settled 2026-09-02; supersedes the 2026-07-02 "amendment pending" mark.)*

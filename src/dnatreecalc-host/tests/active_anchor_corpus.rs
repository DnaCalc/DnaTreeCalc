...
struct AnchorCase {
    ...
    caller: String,
    reference: String,
    expect: AnchorExpectation,
}

#[derive(Debug, Deserialize)]
struct AnchorExpectation {
    outcome: String,
    target: String,
...
#[test]
fn active_anchor_corpus_executes_ancestor_anchors_through_direct_oxcalc_context() {
    let theme = load_theme(repo_corpus_path("references/anchors-raw-active.json"));
    assert_eq!(theme.schema_version, "treecalc-corpus-v1");
    assert_eq!(theme.theme, "references/anchors-raw-active");
    ...
    for case in &theme.cases {
        ...
        assert!(
            ...
            "{} activates unexpected anchor reference {}",
            case.id,
            case.reference
        );
        let mut workspace = workspaces
            ...
            .clone();
        prepare_anchor_case_workspace(&mut workspace, case);
...
fn prepare_anchor_case_workspace(workspace: &mut WorkspaceModel, case: &AnchorCase) {
    ...
    if let Some(target) = workspace.nodes.get_mut(&case.expect.target) {
        target.content = NodeContent::Constant(case.expect.published_value.clone());
    }
    workspace
        .nodes
        .get_mut(&case.caller)

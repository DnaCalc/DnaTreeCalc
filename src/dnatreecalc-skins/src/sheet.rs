...
fn SheetView(cx: SkinContext<SheetState>) -> impl IntoView {
    ...
    let root_keydown = move |ev: leptos::ev::KeyboardEvent| {
        ...
        if event_target_is_text_entry(&ev) || bar_editing.get_untracked() {
            return;
        }
...
/* The `.dtc-grid` / `.dtc-grid__cell` surface styles moved to the shared
   `grid_canvas::GRID_CANVAS_CSS`, concatenated into this lens' stylesheet at
   mount (K1a extraction). */
...
mod tests {
    ...
    // `grid_interest_window_maps_scroll_to_clamped_bounds` moved with the pure
    // fn into `crate::grid_canvas` (K1a extraction).

...
use dnacalc_bench_host::domain::ids::FormulaSpaceId;
use dnacalc_bench_host::state::{FormulaSpaceState, OneCalcHostState};
use dnacalc_bench_host::test_support::sample_editor_document;
use dnacalc_bench_host::ui::editor::commands::{EditorCommand, EditorInputEvent, EditorInputKind};
use dnacalc_bench_host::ui::editor::state::EditorSelection;
...
fn ex_25_completion_navigation_updates_selected_proposal_index() {
    ...
    editor_document.completion_proposals.push(
        dnacalc_bench_host::adapters::oxfml::CompletionProposal {
            proposal_id: "proposal-2".to_string(),
            proposal_kind: dnacalc_bench_host::adapters::oxfml::CompletionProposalKind::Function,
            display_text: "SUBTOTAL".to_string(),
            insert_text: "SUBTOTAL(".to_string(),
            replacement_span: Some(dnacalc_bench_host::adapters::oxfml::FormulaTextSpan {
                start: 1,
                len: 3,
            }),
...
fn ex_26_accept_completion_replaces_current_prefix_range() {
    ...
    editor_document.completion_proposals[0].replacement_span =
        Some(dnacalc_bench_host::adapters::oxfml::FormulaTextSpan { start: 1, len: 2 });
    formula_space.editor_document = Some(editor_document);
    formula_space.editor_surface_state.completion_selected_index = Some(0);
    state.formula_spaces.insert(formula_space);

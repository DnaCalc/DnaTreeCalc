...
use dnacalc_bench_host::services::formula_drill_audit::{
...
use dnacalc_bench_host::services::programmatic_testing::{
    default_verification_config, load_verification_config_xml, ProgrammaticComparisonStatus,
};
use dnacalc_bench_host::services::verification_bundle::{
...
fn run_audit_formula_drill(args: Vec<String>) -> ExitCode {
    ...
    let report = if formulas.is_empty() {
        ...
        FormulaDrillAuditReport {
            // "dnaonecalc" kept across the D5 dnacalc-bench-* rename: persisted-format identifier (v1 data compat).

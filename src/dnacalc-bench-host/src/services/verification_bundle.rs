...
fn build_default_excel_render_context(
    ...
    ProgrammaticExcelRenderContext {
        ...
        note: Some(if spreadsheet_xml_extraction.is_some() {
            "SpreadsheetML-backed verification preserves workbook formatting/style evidence but the bench host does not record any separate Excel-side render-locale pin for the observation host.".to_string()
        } else {
...
fn repo_root() -> Result<PathBuf, String> {
    ...
    manifest_dir
        ...
        .ok_or_else(|| "failed to resolve workspace repo root".to_string())
}
 ...
 xmlns:x="urn:schemas-microsoft-com:office:excel">
  ...
  <Worksheet ss:Name="Input">
    ...
    fn materialize_synthetic_compare_ready_replay_includes_scenario_identity() {
        let temp_root = std::env::temp_dir().join(format!(
            "dnacalc-bench-synthetic-compare-ready-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("time")
                .as_nanos()
    ...
    fn materialize_synthetic_compare_ready_projection_includes_projection_identity() {
        let temp_root = std::env::temp_dir().join(format!(
            "dnacalc-bench-synthetic-projection-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .expect("time")
                .as_nanos()
  ...
  "comparison_views": [
    ...
    fn summarize_excel_capture_normalizes_aligned_text_and_error_payloads() {
        let path = std::env::temp_dir().join(format!(
            "dnacalc-bench-capture-aligned-{}-{}.json",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("unix epoch")
